...
tab1, tab2, tab3 = st.tabs(["🩺 Clinical Screening Desk", "📊 Model Performance", "🧠 Interpretability (XAI)"])
...
with tab2:
    st.markdown("<h3 style='color: #1e3d59; padding-bottom: 10px;'>🏆 Global Model Performance Dashboard</h3>",
                unsafe_allow_html=True)
    st.info(
        "The system uses a Hybrid Ensemble Architecture (CNN Features + SVM/LR Soft Voting). Below are the comprehensive evaluation metrics on the unseen test dataset.")
    ...
    with m1:
        st.metric(label="Accuracy", value="91.27%", delta="Overall Correctness")
    with m2:
        st.metric(label="AUC Score", value="0.9679", delta="Excellent Discernment")
    with m3:
        st.metric(label="F1-Score (Weighted)", value="91.27%", delta="Harmonic Mean")
    with m4:
        st.metric(label="Inference Speed", value="179.15 ms", delta="Lightning Fast", delta_color="normal")
    ...
    with m5:
        st.metric(label="Sensitivity (Recall)", value="91.10%", delta="True Positive Rate (TPR)")
    with m6:
        st.metric(label="Specificity", value="91.45%", delta="True Negative Rate (TNR)")
    with m7:
        st.metric(label="PPV (Precision)", value="92.06%", delta="Positive Predictive Value")
    with m8:
        st.metric(label="NPV", value="90.42%", delta="Negative Predictive Value")

    st.markdown("<br>", unsafe_allow_html=True)

    st.markdown("<div style='font-size:15px; color:#2b5876; font-weight:bold;'>⚠️ Error Rates & Confusion Counts</div>",
                unsafe_allow_html=True)
    m9, m10, m11, m12 = st.columns(4)
    with m9:
        st.metric(label="FDR (False Discovery Rate)", value="7.94%", delta="Low False Alarms", delta_color="normal")
    with m10:
        st.metric(label="FNR (Miss Rate)", value="8.90%", delta="Low Missed Cases", delta_color="normal")
    with m11:
        st.metric(label="Correct Predictions (TP/TN)", value="348 / 321", delta="Total Hits (669)", delta_color="off")
    with m12:
        st.metric(label="Errors (FP/FN)", value="30 / 34", delta="Total Misses (64)", delta_color="off")
    ...
    with c1:
        ...
        try:
            st.image(r"C:\Users\admin\DR images\ROC_Curve_Ensemble.png", use_container_width=True)
        except:
            st.warning("⚠️ Please update the path to ROC_Curve.png")

        st.markdown("**Model Learning Curve**")
        try:
            st.image(r"C:\Users\admin\DR images\Learning_Curve.png", use_container_width=True)
        except:
    ...
    with c2:
        ...
        try:
            st.image(r"C:\Users\admin\DR images\PR_Curve_Ensemble.png", use_container_width=True)
        except:
            st.warning("⚠️ Please update the path to PR_Curve_SVM.png")

        st.markdown("**Confusion Matrix**")
        try:
            st.image(r"C:\Users\admin\DR images\Confusion_Matrix_Ensemble.png", use_container_width=True)
        except:
            st.warning("⚠️ Please update the path to Confusion_Matrix_SVM.png")
...
with tab3:
    ...
    with xai_col1:
        ...
        try:
            st.image(r"C:\Users\admin\DR images\SHAP_Summary_Bar.png", caption="Mean |SHAP| Value (Impact Magnitude)",
                     use_container_width=True)
        except:
            st.warning("⚠️ Please update the path to SHAP_Summary_Bar.png")

    with xai_col2:
        st.markdown("**SHAP Impact Distribution**")
        try:
            st.image(r"C:\Users\admin\DR images\SHAP_Summary_Dot.png",
                     caption="Feature Value Distribution & Directionality", use_container_width=True)
        except:
            st.warning("⚠️ Please update the path to SHAP_Summary_Dot.png")
    ...
    with xai_col3:
        ...
        try:
            st.image(r"C:\Users\admin\DR images\SHAP_Waterfall.png", caption="SHAP Waterfall Plot",
                     use_container_width=True)
        except:
            st.warning("⚠️ Please update the path to SHAP_Waterfall.png")

    with xai_col4:
        st.markdown("**Competing Forces Driving the Output**")
        st.markdown("<br>", unsafe_allow_html=True)
        try:
            st.image(r"C:\Users\admin\DR images\SHAP_Force_Plot.png", caption="SHAP Force Plot",
                     use_container_width=True)
        except:
            st.warning("⚠️ Please update the path to SHAP_Force_Plot.png")